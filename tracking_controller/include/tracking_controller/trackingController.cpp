...
namespace controller{
	...
	void trackingController::cmdCB(const ros::TimerEvent&){
		...
		if (this->bodyRateControl_){
			...
			if (verbose_){
				cout << "[trackingController]: Body rates command: " << cmd(0)<<", "<<cmd(1)<<", "<<cmd(2) << endl;
				cout << "[trackingController]: Estimated thrust: " << this->hoverThrust_ << endl;
			}
	...
	void trackingController::thrustEstimateCB(const ros::TimerEvent&){
		...
		if (this->verbose_){
			// cout << "[Kalman]: Estimation variance: " << this->stateVar_ << endl;
			// cout << "test ratio: " << I*I/Ivar << endl;
			// cout << "[Kalman] new estimate is: " << newHoverThrust << endl;
		}
