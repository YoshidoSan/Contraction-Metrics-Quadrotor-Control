...
namespace AutoFlight{
	...
	void dynamicNavigation::trajExeCB(const ros::TimerEvent&){
		if (this->trajectoryReady_){
			...
			Eigen::Vector3d jer = this->trajectory_.getDerivative().getDerivative().getDerivative().at(this->trajTime_) * pow(linearReparamFactor, 3);
			double endTime = this->trajectory_.getDuration()/linearReparamFactor;

			double leftTime = endTime - realTime; 
			tracking_controller::Target target;
			if (leftTime <= 0.0){ // full stop
				...
				else{ // this is when body rates
					target.yaw = atan2(vel(1), vel(0));
					if (vel(0)==0 and vel(1)==0){
						target.yaw_dot = 0;
					}
					else {
						target.yaw_dot = (vel(0)*acc(1) - vel(1)*acc(0))/(pow(vel(0),2) + pow(vel(1),2));
					}
				}
				target.position.x = pos(0);
				target.position.y = pos(1);
